...
function RtMidi_setCoreMidiClientSingleton(client)
    ccall((:RtMidi_setCoreMidiClientSingleton, rtmidi), Cvoid, (Cint,), client)
end
...
function RtMidi_disposeCoreMidiClientSingleton()
    ccall((:RtMidi_disposeCoreMidiClientSingleton, rtmidi), Cvoid, ())
end
...
function rtmidi_get_compiled_api(apis, apis_size)
    ccall((:rtmidi_get_compiled_api, rtmidi), Cint, (Ptr{RtMidiApi}, Cuint), apis, apis_size)
end

function rtmidi_api_name(api)
    ccall((:rtmidi_api_name, rtmidi), Ptr{Cchar}, (RtMidiApi,), api)
end

function rtmidi_api_display_name(api)
    ccall((:rtmidi_api_display_name, rtmidi), Ptr{Cchar}, (RtMidiApi,), api)
end

function rtmidi_compiled_api_by_name(name)
    ccall((:rtmidi_compiled_api_by_name, rtmidi), RtMidiApi, (Ptr{Cchar},), name)
end

function rtmidi_error(type, errorString)
    ccall((:rtmidi_error, rtmidi), Cvoid, (RtMidiErrorType, Ptr{Cchar}), type, errorString)
end

function rtmidi_open_port(device, portNumber, portName)
    ccall((:rtmidi_open_port, rtmidi), Cvoid, (RtMidiPtr, Cuint, Ptr{Cchar}), device, portNumber, portName)
end

function rtmidi_open_virtual_port(device, portName)
    ccall((:rtmidi_open_virtual_port, rtmidi), Cvoid, (RtMidiPtr, Ptr{Cchar}), device, portName)
end
...
function rtmidi_get_port_count(device)
    ccall((:rtmidi_get_port_count, rtmidi), Cuint, (RtMidiPtr,), device)
end

function rtmidi_get_port_name(device, portNumber)
    ccall((:rtmidi_get_port_name, rtmidi), Ptr{Cchar}, (RtMidiPtr, Cuint), device, portNumber)
end

function rtmidi_in_create_default()
    ccall((:rtmidi_in_create_default, rtmidi), RtMidiInPtr, ())
end

function rtmidi_in_create(api, clientName, queueSizeLimit)
    ccall((:rtmidi_in_create, rtmidi), RtMidiInPtr, (RtMidiApi, Ptr{Cchar}, Cuint), api, clientName, queueSizeLimit)
end

function rtmidi_in_free(device)
    ccall((:rtmidi_in_free, rtmidi), Cvoid, (RtMidiInPtr,), device)
end

function rtmidi_in_get_current_api(device)
    ccall((:rtmidi_in_get_current_api, rtmidi), RtMidiApi, (RtMidiPtr,), device)
end

function rtmidi_in_set_callback(device, callback, userData)
    ccall((:rtmidi_in_set_callback, rtmidi), Cvoid, (RtMidiInPtr, RtMidiCCallback, Ptr{Cvoid}), device, callback, userData)
end

function rtmidi_in_cancel_callback(device)
    ccall((:rtmidi_in_cancel_callback, rtmidi), Cvoid, (RtMidiInPtr,), device)
end

function rtmidi_in_ignore_types(device, midiSysex, midiTime, midiSense)
    ccall((:rtmidi_in_ignore_types, rtmidi), Cvoid, (RtMidiInPtr, Bool, Bool, Bool), device, midiSysex, midiTime, midiSense)
end

function rtmidi_in_get_message(device, message, size)
    ccall((:rtmidi_in_get_message, rtmidi), Cdouble, (RtMidiInPtr, Ptr{Cuchar}, Ptr{Csize_t}), device, message, size)
end

function rtmidi_out_create_default()
    ccall((:rtmidi_out_create_default, rtmidi), RtMidiOutPtr, ())
end

function rtmidi_out_create(api, clientName)
    ccall((:rtmidi_out_create, rtmidi), RtMidiOutPtr, (RtMidiApi, Ptr{Cchar}), api, clientName)
end

function rtmidi_out_free(device)
    ccall((:rtmidi_out_free, rtmidi), Cvoid, (RtMidiOutPtr,), device)
end

function rtmidi_out_get_current_api(device)
    ccall((:rtmidi_out_get_current_api, rtmidi), RtMidiApi, (RtMidiPtr,), device)
end

function rtmidi_out_send_message(device, message, length)
    ccall((:rtmidi_out_send_message, rtmidi), Cint, (RtMidiOutPtr, Ptr{Cuchar}, Cint), device, message, length)
end
